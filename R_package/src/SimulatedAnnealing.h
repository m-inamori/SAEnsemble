...
public:
	...
	// for prediction only
	Variables(const std::vector<std::vector<int>>& vars_) : vars(vars_) { }
...
public:
	...
	const Statistics::Vec& predictions() const { return p; }
	const Statistics::Vec& coefficients() const { return b; }
	const Statistics::Vec& observations() const { return y; }
	void write(std::ostream& os) const;
...
public:
	...
	static Statistics::Vec predict(const SAResult& result,
								   const Statistics::Matrix& A);
