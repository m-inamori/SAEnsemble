...
public:
	Predictor(const Statistics::Matrix& X,
			  const Statistics::Vec& y, const Config *conf, int offset_seed);
	~Predictor();
	
	Statistics::Vec predict(const Statistics::Matrix& X,
							const std::string& weight_type="truncate",
							int num_learners=0, double w_exp=0.0) const;
	Statistics::Vec correlations() const;
	std::vector<const Variables *> variables() const;
	
private:
	bool is_valid_prediction_parameters(const std::string& weight_type,
										int num_learners, double w_exp) const;
	bool is_valid_num_learners(int num_learners) const;
